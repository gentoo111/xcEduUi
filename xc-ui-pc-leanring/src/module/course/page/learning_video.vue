  ...
  export default {
    ...
    methods: {
       ...
       * {
            ...
            "code": 10000,
            "message": null,
            "fileUrl": "8/c/8c137ade94040edb2ddc1fd5ef8c3fdf/hls/8c137ade94040edb2ddc1fd5ef8c3fdf.m3u8"
          }
       * @param chapter
       */
      study(chapter){
    ...
    created(){
      ...
      systemApi.course_view(this.courseId).then((view_course)=>{
        ...
        if(view_course[this.courseId]){//view_course[this.courseId] 课程信息
          let teachplanString  = view_course[this.courseId].teachplan
          //console.log(teachplanString)
          if(teachplanString){

            let teachplan=  JSON.parse(teachplanString)
            ...
            //debugger
            if(this.chapter == '0'){
              //取出第一个课程计划，从 this.teachplanList取出第一个课程计划
              let firstTeachplan = this.teachplanList[0];
              //取出二级课程计划的第一个
              let secondTeachplan = firstTeachplan.children[0].id;
              this.study(secondTeachplan)
            }
          }
        }
